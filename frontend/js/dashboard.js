...
function initDashboard() {
  ...
  // Racha
  const racha = user.racha ?? 7;
  window.SynapseState.racha = racha;
  const rachaEl  = document.getElementById('stat-racha');
  const streakEl = document.getElementById('streak-count');
  if (rachaEl)  rachaEl.textContent  = racha;
  if (streakEl) streakEl.textContent = racha;

  // Sync materia selector
  ...
  console.log('🧠 Synapse Dashboard v2.0 — Usuario:', user.nombre);
}
...
function renderProgressView() {
  ...
  // Materia progress bars
  const list = document.getElementById('prog-materias-list');
  if (!list) return;
  const materias = [
    { name: 'Matemáticas', emoji: '📐', pct: 65 },
    { name: 'Física',       emoji: '⚛️', pct: 40 },
    { name: 'Historia',     emoji: '📜', pct: 80 },
    { name: 'Química',      emoji: '🧪', pct: 25 },
    { name: 'Biología',     emoji: '🧬', pct: 55 },
  ];

  list.innerHTML = materias.map(m => `
    <div style="display:flex;align-items:center;gap:var(--sp-md)">
      <span style="font-size:1.2rem;width:28px;text-align:center">${m.emoji}</span>
      <div style="flex:1">
        <div style="display:flex;justify-content:space-between;margin-bottom:4px">
          <span style="font-size:0.88rem;font-weight:600">${m.name}</span>
          <span style="font-size:0.82rem;font-weight:700;color:var(--clr-primary-l)">${m.pct}%</span>
        </div>
